...
/**
 * Sidebar — a collapsible app-shell side navigation panel. Renders as a
 * fixed desktop sidebar (offcanvas/icon-collapse/none) or, on mobile, as
 * a Sheet that slides in. Wrap your app in SidebarProvider once; every
 * other piece reads shared state from it via useSidebar().
 *
 * Usage:
 *   <SidebarProvider>
 *     <Sidebar>
 ...
 *   - src/lib/primitives/use-render.ts, merge-props.ts (the `render` prop, ref merging - SidebarGroupLabel/Action, SidebarMenuButton/Action, SidebarMenuSubButton)
 *   - src/hooks/use-mobile.ts (isMobile breakpoint detection)
 *   - src/components/ui/button.tsx, input.tsx, separator.tsx, skeleton.tsx
 *   - src/components/ui/sheet.tsx (the mobile sidebar)
 *   - src/components/ui/tooltip.tsx (collapsed-icon-mode tooltips)
 */
